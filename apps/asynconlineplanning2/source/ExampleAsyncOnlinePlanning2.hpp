...
/**
 * @brief Get an example parameter set for the async online planner.
 * @param[in] timestamp The timestamp to be set for the input data.
 * @return Example parameter set.
 */
inline mpsv::planner::AsyncOnlinePlannerParameterSet GetParameterSet(double timestamp){
    mpsv::planner::AsyncOnlinePlannerParameterSet parameterSet;

    // async online planner
    parameterSet.timestamp                                                               = timestamp;
    parameterSet.timeoutInput                                                            = 10.0;

    // online planner
    parameterSet.onlinePlanner.predictInitialStateOnReset                                = true;
    parameterSet.onlinePlanner.maxComputationTimePathOnReset                             = 1.0;
    parameterSet.onlinePlanner.maxComputationTimeMotionOnReset                           = 1.0;
    parameterSet.onlinePlanner.maxComputationTimePath                                    = 2.0;
    parameterSet.onlinePlanner.maxComputationTimeMotion                                  = 3.0;
    parameterSet.onlinePlanner.additionalAheadPlanningTime                               = 0.2;
    parameterSet.onlinePlanner.additionalTrajectoryDuration                              = 1.0;
    parameterSet.onlinePlanner.timeKeepPastTrajectory                                    = 1.0;

    // model
    parameterSet.sequentialPlanner.model.matF                                            = {-0.031604977390302, 0.0, 0.0, 0.233741474945168, 0.0, 0.0, 0.0, 0.0, 0.133462292221887, -0.020792892088185, 0.0, 0.0, 0.0, -0.041708610292712, 0.017242263124379, 0.0, -0.251243228165994, -0.000476044356140, 0.0, 0.0, 0.005297185500344, 0.0, -0.167981638498434, 0.497442136687157, 0.0, 0.000017966020615, -0.023528504195578, 0.0, 0.000108223241795, 0.000649602175186, 0.0, 0.0, -0.000002281767319, 0.0, 0.000072358238719, -0.678801229030825};
    parameterSet.sequentialPlanner.model.matB                                            = {0.000237948834266, -0.000004551592718, 0.000010003488944, -0.000009313932115, 0.000215194147058, -0.000024957572224, -0.000002202124158, -0.000002930260852, 0.000043018345190};
    parameterSet.sequentialPlanner.model.vecTimeconstantsXYN                             = {0.2, 0.2, 0.2};
    parameterSet.sequentialPlanner.model.vecTimeconstantsInput                           = {0.5, 0.5, 0.5};
    parameterSet.sequentialPlanner.model.lowerLimitXYN                                   = {-630.0, -495.0, -675.0};
    parameterSet.sequentialPlanner.model.upperLimitXYN                                   = {630.0, 495.0, 675.0};

    // cost map
    parameterSet.sequentialPlanner.costMap.modBreakpoints                                = 10;
    parameterSet.sequentialPlanner.costMap.resolution                                    = 0.05;
    parameterSet.sequentialPlanner.costMap.distanceScale                                 = 5.0;
    parameterSet.sequentialPlanner.costMap.distanceDecay                                 = 0.02;

    // geometry
    parameterSet.sequentialPlanner.geometry.collisionCheckMaxPositionDeviation           = 0.100000000000000;
    parameterSet.sequentialPlanner.geometry.collisionCheckMaxAngleDeviation              = mpsv::math::deg2rad(5.0);
    std::vector<std::array<double,2>> vertices                                           = {{3.25,1.65},{-3.25,1.65},{-3.25,-1.65},{3.25,-1.65}};
    parameterSet.sequentialPlanner.geometry.vehicleShape.Create(vertices);
    parameterSet.sequentialPlanner.geometry.skeletalPoints.clear();
    parameterSet.sequentialPlanner.geometry.skeletalPoints.push_back({3.0, 0.0});
    parameterSet.sequentialPlanner.geometry.skeletalPoints.push_back({-3.0, 0.0});
    ...
    // motion panner
    parameterSet.sequentialPlanner.motionPlanner.sampletime                              = 0.05;
    ...
    parameterSet.sequentialPlanner.motionPlanner.regionOfAttraction.rangeXYN             = {10.0, 10.0, 10.0};
    parameterSet.sequentialPlanner.motionPlanner.controller.matK                         = {0.054, 0.0, 0.0, 0.531, 0.0, 0.0, 1.785, 0.0, 0.0, 2.35, 0.0, 0.0, 0.0, 0.054, 0.0, 0.0, 0.531, 0.0, 0.0, 1.785, 0.0, 0.0, 2.35, 0.0, 0.0, 0.0, 0.054, 0.0, 0.0, 0.531, 0.0, 0.0, 1.785, 0.0, 0.0, 2.35};
    parameterSet.sequentialPlanner.motionPlanner.controller.maxRadiusX                   = 10.0;
    parameterSet.sequentialPlanner.motionPlanner.controller.maxRadiusY                   = 6.0;
    parameterSet.sequentialPlanner.motionPlanner.controller.maxRadiusPsi                 = 1.0;
    parameterSet.sequentialPlanner.motionPlanner.controller.minRadiusPosition            = 2.0;
    return parameterSet;
}


/**
 * @brief Get an example input data for the async online planner.
 * @param[in] timestamp The timestamp to be set for the input data.
 * @return Example input data.
 */
inline mpsv::planner::AsyncOnlinePlannerInput GetInput(double timestamp){
...
std::string GetDataFilename(void){
    ...
    std::filesystem::path filePath = applicationPath / "dataExamplePlanner";
