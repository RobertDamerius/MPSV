...
static std::string ToPrintableString(std::string s){
    std::string result;
    for(auto&& c : s){
        if((c >= ' ') || (c <= '~')){
            result.push_back(c);
...
int32_t UDPSocket::BindToDevice(std::string deviceName){
    deviceName = ToPrintableString(deviceName);
    #ifdef _WIN32
    (void)deviceName;
    return 0;
    #else
...
int32_t UDPSocket::SendTo(IPAddress destination, uint8_t *bytes, int32_t size){
    sockaddr_in addr;
