...
class AsyncOnlinePlanner {
    public:
         ...
         */
        bool Initialize(int16_t pathMaxNumNodes, uint32_t pathMaxNumSamples, int16_t motionMaxNumNodes, uint32_t motionMaxNumSamples, int32_t threadPriority, int32_t ompNumThreads, int32_t ompDynamic) noexcept {
            terminate = false;
            mtxDatabase.lock();
            database.Clear();
            mtxDatabase.unlock();
            ...
            workerThread.Start(std::bind(&AsyncOnlinePlanner::WorkerThreadRun, this), threadPriority, ompNumThreads, ompDynamic);
            return true;
        }
